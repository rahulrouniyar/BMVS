...
class Mainapp_staff(QMainWindow, ui):
    ...
    def Handle_Buttons(self):
        ...
        self.pushButton_19.clicked.connect(self.Update_Candidate_Votes)
        self.pushButton_20.clicked.connect(self.Update_Party_Votes)
        ...
        if((self.radioButton_5.isChecked()==False and self.radioButton_6.isChecked()==False and 
            ...
            try:
                ...
                if citizenship_repeat:
                    ...
                    sql_fetch_voterid = '''SELECT name,voter_id,election_area from voter_details where citizenship_no = %s'''
                    self.cur.execute(sql_fetch_voterid,(citizenship_id,))

                    details = self.cur.fetchone()

                    display_name = str(details[0])
                    display_voterid = str(details[1])
                    display_votingarea = str(details[2])

                    self.cur.close()
                    self.database.close()

                    self.tabWidget_3.setCurrentIndex(1)
                    self.label_29.setText(display_name)
                    self.label_30.setText(display_voterid)
                    self.label_38.setText(display_votingarea)
    ...
    def DisplayCandidate(self):
        ...
        # self.tableWidget.setHorizontalHeaderLabels(['Candidate','Party','Symbol','Vote'])
        ...
        try:
            ...
            if candidate_no == 0:
                ...
                for data in candidate_list:
                    ...
                    # pushbutton4 = QPushButton(self.tableWidget)
    ...
    def On_SelectionChanged(self,selected,deselected):
        ...
        # pushbutton5 = QPushButton(self.tableWidget)
        ...
        for ix in selected.indexes():
            ...
            if (ix.column() == column):
                ...
                # print(self.tableWidget.cellWidget(int(ix.row()),0).text())
        

    def Update_Candidate_Votes(self):
        position = int(self.label_59.text())
        candidate = str(self.tableWidget.cellWidget(position,0).text())
        try:
            self.database = mysql.connector.connect( host='localhost',user='root',password='',database = dbname)
            self.cur = self.database.cursor()           
        except mysql.connector.Error as err:
            ...
            self.database.commit()
            self.statusBar().showMessage("Vote to candidate successfull")
            self.cur.close()
            self.database.close()
            self.Party_Voting()
    ...
    def Update_Party_Votes(self):
        position = int(self.label_63.text())
        party = str(self.tableWidget_2.cellWidget(position,0).text())
        voter_id = int(self.lineEdit_10.text())
        try:
            self.database = mysql.connector.connect( host='localhost',user='root',password='',database = dbname)
            self.cur = self.database.cursor()           
            ...
            self.database.commit()
            self.statusBar().showMessage("Vote to party successfully recorded.")
            self.cur.close()
            self.database.close()     
    ...
    def Biometrics_Registration(self):
        if(self.tabWidget.currentIndex() == 1):
            ...
            voterId = int(self.label_30.text())
    ...
    def Biometrics_Verification(self):
        ...
        displayLabelResult = self.label_55
        ...
        if (int(self.tabWidget.currentIndex())==3):
            ...
            displayLabelResult = self.label_57
            displayLabelResult_failed = self.label_57
